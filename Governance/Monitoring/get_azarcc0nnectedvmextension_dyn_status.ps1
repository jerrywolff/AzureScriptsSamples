        ...
        $updateStatus = Get-AzConnectedMachineExtension -MachineName "$($server.Name)" -ResourceGroupName "$($server.ResourceGroupName)"
          ...
          foreach($extensionstatus in $updateStatus)
          {
               $arcextensionobj = new-object PSobject  
                 ...
                 $arcextensionobj  | Add-Member -MemberType NoteProperty -name machinename -value $($server.Name)
             
            foreach($extprop in ($extensionstatus | gm | where membertype -eq 'property')|select name)
            {
                  $arcextensionobj  | Add-Member -MemberType NoteProperty -name $($extprop.name)  -value $($extensionstatus."$($extprop.name)")
...
"@
 ...
 $azarconnectedextdata = ($azarcconnectedextstatus | select machinename, AutoUpgradeMinorVersion ,`      
...
Tag,`                           
Type,`                          
TypeHandlerVersion   | ConvertTo-Html -Head $CSS )  | out-file "c:\temp\azarcconnectedextensionreport.html" 
...
$csvresults = ($azarcconnectedextstatus | select machinename, AutoUpgradeMinorVersion ,`      
EnableAutomaticUpgrade ,`       
ForceUpdateTag ,`                
Id,`                            
InstanceViewName,`              
...
Tag,`                           
Type,`                          
TypeHandlerVersion )| export-csv $resultsfilename -notypeinformation 
