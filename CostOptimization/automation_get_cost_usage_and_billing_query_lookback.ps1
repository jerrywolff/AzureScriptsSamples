...
######################  Commercial 
       ...
       $requesturi = "https://management.azure.com/$billingaccount/providers/Microsoft.CostManagement/query?api-version=2022-10-01"
        ...
        # Send the request to the API  Recommendations collection  
            ...
            $resultsfilename = "$($subcription.name)_billingdata.csv"

        $response = Invoke-RestMethod -Uri $requestUri -Headers ($body.Headers)  -Method GET -ErrorAction silentlycontinue


        # Output the response
         $($response.value).properties | export-csv $resultsfilename -NoTypeInformation -Append 
  ...
  ########################  Usage collection 
            ...
            $resultsfilename1 = "$($subcription.name)_usagedetailsconsumption.csv"
             $($usageresponse.value).properties | export-csv $resultsfilename1 -NoTypeInformation  
